...
def delete_calibration(subject_id, gesture):
    _delete("calibration", "(subject_id = %s AND calibration_gesture = %s)",
            (subject_id, gesture,))


def delete_data(subject_id, gesture, repetition):
    _delete("data", "(subject_id = %s AND gesture = %s AND repetition = %s)",
...
def clear_existing_data(meta_data):
    if exists('data', meta_data):
        row_deleted = delete_data(
            subject_id=meta_data['subject_id'], gesture=meta_data['gesture'],  repetition=meta_data['repetition'])
